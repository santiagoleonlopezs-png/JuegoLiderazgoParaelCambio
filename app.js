...
function effects(){
 ...
 const map={
 "ADKAR":{Conciencia:4,Conocimiento:4,Compromiso:2},"Lean Change":{Confianza:4,Compromiso:4,Capacidad:2},
 "Moving Motivators":{Compromiso:7,Confianza:3},"Delegation Poker":{Confianza:4,Compromiso:5},
 "Campañas de comunicación-difusión":{Conciencia:9,Conocimiento:2,Energía:-1},"TRIZ":{Confianza:2,Conciencia:2},
 "Nudges":{Capacidad:3,Compromiso:2},"Redes de influencia":{Compromiso:6,Confianza:2},
 "Storytelling":{Conciencia:5,Compromiso:4},"Co-creación":{Confianza:5,Compromiso:6,Energía:-3},
 "Aprendizaje experiencial":{Conocimiento:5,Capacidad:8,Energía:-2},"Coaching":{Capacidad:5,Confianza:3},
 "Reconocimiento":{Energía:5,Compromiso:5},"Comunidades de práctica":{Conocimiento:5,Capacidad:5,Confianza:2},
 "Lecciones aprendidas":{Conocimiento:5,Capacidad:4},"Comunicación segmentada":{Conciencia:5,Conocimiento:3},
 "Respaldo directivo":{Conciencia:5,Confianza:6,Compromiso:5},"Grupos focales":{Confianza:5,Conciencia:3,Compromiso:4,Energía:-1}
 };
 const fit=[
 ...
 names.forEach(n=>{
   const f=fit[n]??.7;
   Object.entries(map[n]).forEach(([k,v])=>e[k]+=v*f);
   if(f<=.35){e.Energía-=3;e.Confianza-=2;e.Compromiso-=1;}
 });
 ...
 if(highest&&highest[1]>=40&&highest[0]!==target){e.Confianza-=2;e.Energía-=2;}
 if(names.includes("Grupos focales")&&names.includes("Lean Change")){e.Confianza+=3;e.Compromiso+=2;}
 if(names.includes("Respaldo directivo")&&names.includes("Campañas de comunicación-difusión"))e.Conciencia+=4;
 if(names.includes("Redes de influencia")&&names.includes("Reconocimiento")){e.Compromiso+=3;e.Confianza+=2;}
 if(names.includes("Aprendizaje experiencial")&&names.includes("Comunidades de práctica")){e.Capacidad+=4;e.Conocimiento+=2;}
 if(names.includes("Co-creación")&&names.includes("Delegation Poker")){e.Compromiso+=3;e.Confianza+=2;}
 if(names.includes("Co-creación")&&state.metrics.Energía<50){e.Energía-=6;e.Compromiso-=2;}
 if(names.includes("Delegation Poker")&&state.metrics.Capacidad<45)e.Confianza-=3;
 if(names.includes("Aprendizaje experiencial")&&state.metrics.Compromiso<40)e.Energía-=3;
 if(names.includes("Campañas de comunicación-difusión")&&state.metrics.Conciencia>75){e.Energía-=5;e.Confianza-=2;}
 if(names.includes("Reconocimiento")&&state.metrics.Confianza<40)e.Confianza-=3;
 const spend=selectedCost();
 if(names.length>=4){e.Energía-=4;e.Confianza-=2;}
 if(spend>=70){e.Energía-=5;e.Compromiso-=2;}
...
async function execute(){
 ...
 clearInterval(tick); let e=effects(), before={...state.metrics};
 vars.forEach(v=>state.metrics[v]=Math.max(0,Math.min(100,state.metrics[v]+e[v])));
 // appropriation emerges from the system
 let emerg=(state.metrics.Conciencia*.22+state.metrics.Compromiso*.25+state.metrics.Conocimiento*.17+state.metrics.Capacidad*.25+state.metrics.Confianza*.11);
 state.metrics.Apropiación=Math.max(0,Math.min(100,state.metrics.Apropiación*.65+emerg*.35));
